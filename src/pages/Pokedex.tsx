...
const Pokedex = () => {  
  ...
  useEffect( () => {
    const pokeArray = data?.pokemon_v2_pokemonspecies.map( (poke: any) => {
      return {
        id: poke.id,
        name: poke.name,
        imageUrl: `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${poke.id}.png`,
        type1: poke.pokemon_v2_pokemons[0]?.pokemon_v2_pokemontypes[0]?.pokemon_v2_type.name,
        type2: poke.pokemon_v2_pokemons[0]?.pokemon_v2_pokemontypes[1]?.pokemon_v2_type.name
